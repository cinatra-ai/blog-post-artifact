...
describe("the package draws through the one shared sanitizer", () => {
  ...
  it("imports the sanitizer at its leaf specifier, in exactly one module", () => {
    ...
    expect(importers[0].endsWith("/renderers/text-view.ts")).toBe(true);
  });
  ...
  it("builds a drawable view in ONE module, and its html comes from the sanitizer call itself", () => {
    ...
    expect(builders[0].endsWith("/renderers/text-view.ts")).toBe(true);
    ...
    expect(assignments[0]).toBe(
      'withoutPictures(renderSanitizedMarkdown(text, { demoteHeadings: true }))',
    );
    // The ONE transformation wrapped around the sanitizer call is the ruling's
    // picture removal, written in this same module and touching nothing but the
    // one image element the sanitizer is able to emit. It is not a second
    // sanitizer: it decides nothing about what is safe.
    expect(/export function withoutPictures\(/.test(view)).toBe(true);
    const removal = /html\.replace\(([^,]+),/.exec(view)?.[1]?.trim();
    expect(removal).toBe('/<img\\b[^>]*>/gi');
    ...
    // itself. The one injection sits in `TextBody`, whose only argument is that
    // value — so the guard reads both halves: what is injected is the
    ...
    // road.)
    const component = readFileSync(`${SRC}/renderers/text-document.tsx`, "utf8");
  ...
  it("injects html in exactly ONE place, ONCE, by no other road this guard knows of", () => {
    ...
    expect(file.endsWith("/renderers/text-document.tsx")).toBe(true);
    expect(count).toBe(1);
  });
});
